  ...
  return (
    <div className="relative w-full org-menu-container">
      <button
        ...
        title={orgName}
        className={cn(
          "group/button flex items-center border border-foreground/15 bg-sidebar text-left text-sm font-semibold text-sidebar-foreground outline-none transition-all hover:border-foreground cursor-pointer",
          collapsed ? "size-10 justify-center" : "h-12 w-full gap-x-2 px-3"
        )}
      >
        <span className="flex size-8 shrink-0 items-center justify-center bg-[#facc15] font-mono text-sm font-bold text-black">{initial}</span>
      ...
      {isOpen && (
        <div className={cn(
          ...
          <div className="flex items-center gap-2 px-2 py-2">
            <span className="flex size-8 shrink-0 items-center justify-center bg-foreground font-mono text-sm font-semibold text-background">{userInitial}</span>
...
export function Sidebar({ sections, orgName, user, collapsed = false }: { sections: NavSection[]; orgName: string; user: CurrentUser; collapsed?: boolean }) {
  ...
  return (
    <aside className={cn("flex h-full shrink-0 flex-col bg-transparent p-2 transition-[width] duration-300 ease-in-out", collapsed ? "w-[74px]" : "w-[260px]")}>
      <div className="relative flex size-full flex-col overflow-visible border border-foreground/15 bg-sidebar">
        ...
        <nav className="flex-1 space-y-4 overflow-y-auto px-2 py-3">
          {sections.map((section) => <div key={section.label} className="space-y-1">
            {!collapsed && <p className="px-3 py-1.5 font-mono text-[10px] font-bold uppercase tracking-widest text-sidebar-foreground/40">[ {tx(locale, section.label)} ]</p>}
            {section.items.map((item) => {
              ...
              return <Link key={item.href} href={item.href} title={collapsed ? label : undefined} className={cn("group flex h-10 items-center gap-3 text-sm font-medium transition-all duration-200", collapsed ? "size-10 justify-center px-2" : "w-full justify-start px-3", active ? "bg-foreground text-background" : "text-sidebar-foreground hover:bg-[#facc15]/25")}><Icon className={cn("size-5 shrink-0 transition-colors", active ? "text-background" : "text-sidebar-foreground/70 group-hover:text-sidebar-foreground")} />{!collapsed && <span className="truncate">{label}</span>}{!collapsed && active && <span className="ml-auto size-[8px] bg-[#facc15]" />}</Link>;
            })}
          </div>)}
        </nav>
      </div>
...
export function DashboardHeader({ breadcrumb, onToggle }: { breadcrumb: string; onToggle: () => void }) {
  ...
  return (
    <header className="flex h-16 shrink-0 items-center gap-3 border-b border-foreground/15 bg-background px-6">
      <button onClick={onToggle} aria-label={tx(locale, "Toggle Sidebar")} className="flex size-9 cursor-pointer items-center justify-center border border-transparent text-muted-foreground transition-all hover:border-foreground hover:text-foreground">
        <PanelLeftIcon className="size-5" />
      </button>
      <span className="h-5 w-px bg-foreground/15" />
      <nav aria-label="breadcrumb" className="flex-1">
        <span className="font-mono text-xs font-bold uppercase tracking-widest text-foreground/70">{tx(locale, breadcrumb)}</span>
      </nav>
    </header>
  );
}
