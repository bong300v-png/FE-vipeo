...
function Metric({ title, value, description }: { title: string; value: string; description: string }) {
  return (
    <Card className="group relative rounded-none border-foreground/15 shadow-none transition-colors hover:border-foreground">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardDescription className="font-mono text-[11px] font-bold uppercase tracking-widest">{title}</CardDescription>
          <span className="size-[10px] border border-foreground bg-background transition-colors group-hover:bg-[#facc15]" />
        </div>
        <CardTitle className="text-5xl font-black tracking-tighter">{value}</CardTitle>
      </CardHeader>
      <CardContent>
...
export function DashboardContent() {
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      ...
      <Link href="/ai" className="group relative block overflow-hidden border border-foreground bg-foreground p-8 text-background transition-colors">
        <div className="flex items-center gap-2">
          <span className="size-[10px] bg-[#facc15]" />
          <p className="font-mono text-xs font-bold uppercase tracking-widest text-background/60">{tx(locale, "AI Director")}</p>
        </div>
        <h2 className="mt-4 max-w-2xl text-3xl font-black tracking-tighter text-balance lg:text-4xl">{tx(locale, "Describe the video outcome. Vipeo picks the workflow.")}</h2>
        <p className="mt-4 max-w-2xl text-base leading-relaxed text-background/60">{tx(locale, "Start with a topic, product, language, and platform. The composer shows skill fit, cost, rights gates, and mock progress.")}</p>
        <span className="mt-6 inline-flex items-center gap-2 bg-[#facc15] px-5 py-2.5 font-mono text-xs font-bold uppercase tracking-widest text-black transition-transform group-hover:translate-x-1">
          {tx(locale, "Create with AI")} {"->"}
        </span>
      </Link>

      <section id="skills" className="space-y-4">
        <div><p className="mb-1 font-mono text-[11px] font-bold uppercase tracking-widest text-foreground/40">[ {tx(locale, "Library")} ]</p><h2 className="text-2xl font-black tracking-tighter">{tx(locale, "Skill library")}</h2><p className="text-sm text-muted-foreground">{tx(locale, "Launch skills plus clearly marked roadmap workflows.")}</p></div>
      ...
      <section id="jobs" className="space-y-4">
        <div><p className="mb-1 font-mono text-[11px] font-bold uppercase tracking-widest text-foreground/40">[ {tx(locale, "Jobs")} ]</p><h2 className="text-2xl font-black tracking-tighter">{tx(locale, "Recent jobs")}</h2></div>
        <div className="overflow-x-auto border border-foreground/15 bg-card">
